...
Процедура Подготовить( Знач пИсточник = "", Знач пИмяПодготовленногоФайла = "" ) Экспорт
	...
	Если пИмяПодготовленногоФайла = "" Тогда
		
		ИмяПодготовленногоФайла = "tmp\ForSpellChecking.txt";
		
	Иначе
		
		ИмяПодготовленногоФайла = пИмяПодготовленногоФайла;
		
	КонецЕсли;

	подготовленныйТекст.Записать( ИмяПодготовленногоФайла );
...
Функция ПодготовитьСтроку( Знач пСтрока )
	...
	Для ц = 1 По СтрДлина( пСтрока ) Цикл
		...
		Если ЭтоБуква( текСимвол ) Тогда
			...
			следСимвол     = Сред( пСтрока, ц + 1, 1 );
			следСимволВРег = ВРег( следСимвол ) = следСимвол;
			
			Если ВРег( текСимвол ) = текСимвол
				И этоСлово
				И ( Не предСимволВРег
				ИЛИ Не следСимволВРег ) Тогда // Начало слова внутри строки

...
Функция РазделителиБлоков()
	...
	ДобавитьСлова_Конец( РазделителиБлоков );
...
Функция УдалитьПробелыПередСкобкой(Знач пСтрока)
	...
	Если КодЗаканчиваетсяНа( строкаСокрП, "Если", Истина )
		ИЛИ КодЗаканчиваетсяНа( строкаСокрП, "If", Истина )
		ИЛИ КодЗаканчиваетсяНа( строкаСокрП, "ИначеЕсли", Истина )
		ИЛИ КодЗаканчиваетсяНа( строкаСокрП, "ElsIf", Истина )
		ИЛИ КодЗаканчиваетсяНа( строкаСокрП, "#Если", Истина )
		ИЛИ КодЗаканчиваетсяНа( строкаСокрП, "#If", Истина )
		ИЛИ КодЗаканчиваетсяНа( строкаСокрП, "#ИначеЕсли", Истина )
		ИЛИ КодЗаканчиваетсяНа( строкаСокрП, "#ElsIf", Истина )
		ИЛИ КодЗаканчиваетсяНа( строкаСокрП, "Пока", Истина )
		ИЛИ КодЗаканчиваетсяНа( строкаСокрП, "While", Истина ) Тогда
...
Процедура РазбитьВыраженияПоПереносам(пТекСтрока, строкаИзменена)
	...
	позицияРазрыва = СтрДлина(пТекСтрока) + 1;
	фрагментРазрыва = "";
	...
	Для каждого цЭлемент Из КлючевыеСловаЗавершения() Цикл
		...
		найденнаяПозиция = ПозицияРазрыва(пТекСтрока, фрагмент, искатьСлово);
		
		Если Не найденнаяПозиция = 0
			И Не ПустаяСтрока( Лев(пТекСтрока, найденнаяПозиция - 1)) Тогда
	...
	Для каждого цЭлемент Из КлючевыеСловаНачала() Цикл
		...
		позицияФрагмента = ПозицияРазрыва(пТекСтрока, фрагмент, искатьСлово);
		
		Если позицияФрагмента > 1
			И Не ПустаяСтрока(Лев(пТекСтрока, позицияФрагмента - 1)) Тогда
